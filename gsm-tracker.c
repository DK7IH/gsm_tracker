///////////////////////////////////////////////////////////////////  
/*                    GPS-Decoder + GSM tracker w. OLED SH1306   */
/*                    GPS-NMEA module, SIM800-L                  */
///////////////////////////////////////////////////////////////////
...
int oled_putstringx(int, int, char *, int);
...
//Data for buffers of USART1 and USART2
char *msg_usart2_rx;
char *msg_usart1_tx, *msg_usart1_rx;
char *buf; 
...
extern "C" void USART1_IRQHandler(void)
{
	...
	{
	    msg_usart1_rx[bufcnt1++] = rxdata;
	    msg_usart1_rx[bufcnt1] = 0;
...
extern "C" void USART2_IRQHandler(void)
{
	...
	if((rxdata >= 32) && (rxdata < 128))
	{
	    msg_usart2_rx[bufcnt2++] = rxdata;
	    msg_usart2_rx[bufcnt2] = 0;
	    if(bufcnt2 > MAXBUFFER)
	    {
			bufcnt2 = 0;
			scomplete2 = 1;
		}	
...
int oled_putstringx(int col, int row, char *s, int inv)
{
	int t1;
	
	int sx = col, sy = row; //Screen coordinates for output
	
    for(t1 = 0; s[t1] && t1 < 160; t1++) //Display string
	{
	    if((s[t1] > 31) && (s[t1] < 127))
	    {
	        oled_putchar1(sx * FONTW, sy, s[t1], inv);
	    }
...
{	
	...
	char *smstext;                //Buffer for SMS ready to send
	uint32_t runcseconds_old = 0; //Variable for seconds counter
	int y2;
	
	uint16_t pin_input;
	
	//Special characters for SMS transmission via USART
	char str_crlf[] =  {13, 10, 0};  //Carriage return + line feed 
	char str_eosms[] = {26, 0};      //End of message indicator 0x1A
	char str_quot[] =  {34, 0};      //"-Character
	
	//Allocate memory for strings
	msg_usart1_rx = (char*)malloc(MAXBUFFER + 1);
	msg_usart1_tx = (char*)malloc(MAXBUFFER + 1);
	msg_usart2_rx = (char*)malloc(MAXBUFFER + 1);
	smstext = (char*)malloc(MAXBUFFER + 1);
    ...
    //////////////////////////////////////////////
    // Setup USART1 for SIM800-L module
    //////////////////////////////////////////////
    ///////////////
    // TX Section  
    //////////////  
    ...
    GPIOA->MODER &= ~(0b1111 << 4); //Reset bits 4:5 for PA2 and 6:7 for PA3
    GPIOA->MODER |=  (0b1010 << 4); //Set   bits 4:5 for PA2 and 6:7 for PA3 to alternate mode (10)

    //Set pin modes as high speed
    GPIOA->OSPEEDR |= 0xA0 << 4; //Set pin 2/3 to fast speed mode (0b10) << 4

    //Choose AF7 for USART2 in Alternate Function registers
    GPIOA->AFR[0] |= (0x07 << 8); // for pin A2
    GPIOA->AFR[0] |= (0x07 << 12); // for pin A3
    ...
    //////////////////////////////////////////
    // Set up TIM2 as counter for seconds
    //////////////////////////////////////////
    ...
    TIM2->PSC = 50000; //Divide system clock (f=50MHz) by 50000 -> update frequency = 1000/s
    TIM2->ARR = 1000;    //Define overrun after 500ms

    //Update Interrupt Enable
    TIM2->DIER |= (1 << 0);
    TIM2->CR1 |= (1 << 0);           //Enable Timer 2 module (CEN, bit0)
    
    NVIC_SetPriority(TIM2_IRQn, 5); //Priority level 2
    NVIC_EnableIRQ(TIM2_IRQn);      //Enable TIM2 IRQ from NVIC
    
    ///////////////////////////////////////////////////////
    //Pin PA0 to 'input' mode with pull-up.
    ///////////////////////////////////////////////////////
    GPIOA->MODER  &= ~(3 << (BUTTON_PIN << 1)); //Set to 00 -> Input mode
    GPIOA->PUPDR  &= ~(3 << (BUTTON_PIN << 1)); //Set to 00	-> No pullup nor pulldown
    GPIOA->PUPDR  |=  (1 << (BUTTON_PIN << 1)); //Set to 01 -> Pull up
    ...
    //Init data buffers USARTs 1 and 2, SMS
    ...
    while(1)
    {
        if(scomplete2) //USART2 Transmission from NMEA device completed -> process string
        {
			pointer0 = strstr(msg_usart2_rx, "$GPGLL"); //Find NMEA string identifier in input buffer
			if(pointer0)
			{
				//Save text data
				strcpy(smstext, msg_usart2_rx);
					
				//Display NMEA string on screen	
				oled_cls(0);
			    oled_putstring(0, 0, (char*)"DK7IH GPS Tracker", 0, 0); 
			    oled_putstringx(0, 2, msg_usart2_rx, 0);
        ...
        //Check if alert condition present
		pin_input = ~GPIOA -> IDR; //"0" means "pressed"!
		if(pin_input & (1 << BUTTON_PIN))
		{
			...
			USART2->CR1 &= ~(1 << 13);  //DISanable USART2 - UE, bit 13
			
			while(pin_input & (1 << BUTTON_PIN)) //Wait till switch is released
            {
				pin_input = ~GPIOA -> IDR;
			}	
			
			for(t1 = 0; t1 < MAXBUFFER; t1++) //Init RX buffer
			{
				msg_usart1_rx[t1] = 0;
			}	
			
			//Send SMS
			for(t1 = 0; t1 < MAXBUFFER; t1++) //Init TX buffer
		    {
			    msg_usart1_tx[t1] = 0;
		    }		
			...
			strcpy(msg_usart1_tx, (char*)"AT+CMGF=1");
			strcat(msg_usart1_tx, str_crlf);
            usart1_send(msg_usart1_tx);
            
            //Wait 1 second
            runcseconds_old = runcseconds;
            while(runcseconds < runcseconds_old + 1)
            {
				oled_putnumber(0, 7, runcseconds, -1, 0, 0);
			}	
			
            if(scomplete1) //Transmission from SIM800-L completed -> process string
            {
				y2 = oled_putstringx(0, y2, msg_usart1_rx, 0);
				scomplete1 = 0;
			}	
			...
			if(scomplete1) //Transmission from SIM800-L completed -> process string
            {
				y2 = oled_putstringx(0, y2 + 1, msg_usart1_rx, 0);
				scomplete1 = 0;
			}	
						
			//Send SMS
			strcpy(msg_usart1_tx, (char*)smstext);
			strcat(msg_usart1_tx, str_eosms);
			
			//Set SMS reference number
			strcat(msg_usart1_tx, (char*)"+CMGS: 1");
			usart1_send(msg_usart1_tx);
			...
			if(scomplete1) //Transmission from SIM800-L completed -> process string and display
            {
				oled_putstringx(0, y2 + 1, msg_usart1_rx, 0);
				scomplete1 = 0;
			}				
		    ...
		    //Restore USART settings for reading GPS data
		    USART1->CR1 &= ~(1 << 13);  //Disable USART1
